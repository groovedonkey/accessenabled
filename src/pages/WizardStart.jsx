...
export default function WizardStart() {
  ...
  const handleCreate = async (e) => {
    ...
    setCreating(true);
    try {
      const id = await createAudit(user.uid, {
        url: url.trim(),
        client: client.trim(),
      });
      navigate(`/audit/${id}`);
  ...
  return (
    <div className="dashboard">
      <section className="new-audit-card">
        <h2>
          <ClipboardCheck size={20} /> New guided audit
        </h2>
        ...
        <p className="hint">
          Enter the project URL and name, then run an initial automated scan
          (single or multi-page). The wizard guides you through each failed item
          with fix steps, then the items needing manual review.
        </p>
      </section>
