...
export function applyScanToResults(scan, baseResults) {
  ...
  (scan.violations || []).forEach((v) => {
    violationById[v.id] = v;
  });
  const incompleteById = {};
  (scan.incomplete || []).forEach((v) => {
    incompleteById[v.id] = v;
  });
  const passSet = new Set(scan.passes || []);

  CHECKLIST.forEach((section) => {
    section.items.forEach((item) => {
      const prev = results[item.ref] || {
        status: "untested",
        note: "",
        findings: [],
      };
      ...
      if (item.detector && scan.detectors && scan.detectors[item.detector]) {
        results[item.ref] = applyDetector(
          item,
          scan.detectors[item.detector],
          prev,
        );
        return;
      }
      ...
      if (rules.length === 0) {
        ...
        results[item.ref] = {
          ...prev,
          status: prev.status === "untested" ? "manual" : prev.status,
          auto: false,
        };
        return;
      }
      ...
      rules.forEach((rule) => {
        if (violationById[rule]) {
          sawViolation = true;
          findings.push(toFinding("violation", violationById[rule]));
        }
        if (incompleteById[rule]) {
          sawIncomplete = true;
          findings.push(toFinding("incomplete", incompleteById[rule]));
        }
  ...
  if (item.detector === "accessibilityStatement") {
    return {
      ...
      note:
        ...
        (det.found
          ? `Found: ${det.href}`
          : "No accessibility statement link detected."),
      findings: det.found
        ? []
        : [
            {
              type: "detector",
              summary: 'No "Accessibility Statement" link found in page.',
            },
          ],
      auto: true,
    };
    ...
    return {
      ...
      findings: hasOverlay
        ? [
            {
              type: "detector",
              summary: `Overlay vendor(s): ${det.vendors.join(", ")}`,
            },
          ]
        : [],
      auto: true,
